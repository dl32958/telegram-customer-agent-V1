...
def save_order_to_sheet(order_details: Dict[str, Any], order_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        orders_sheet = client.open("PerfumePal E-commerce").get_worksheet(1)
        if order_id is None:
            order_id = f"ORD{random.randint(100, 999)}"
        today_date = datetime.date.today().isoformat()
        tracking_link = f"https://perfumepal.shop/track_order{order_id}"
        row = [
            order_id,
        ...
        We’re excited to confirm your order with PerfumePal E-commerce. Here are the details:
        ...
        Best regards,
        The PerfumePal E-commerce Team
        """
