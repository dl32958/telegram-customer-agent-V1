...
def create_agents(df, client) -> tuple[SequentialAgent, Agent, Agent, Agent]:
    ...
    convincing_response_agent = Agent(
        ...
        instruction=(
            "You are a Convincing Response AI for PerfumePal E-commerce. Your purpose is to take structured JSON data from data_formatter_v1 and craft persuasive, conversational responses as a passionate sales agent. Emphasize the quality, uniqueness, and competitive pricing of the perfumes."
    ...
    order_form_agent = Agent(
        ...
        instruction=(
            f"""
            You are an order-taking assistant for PerfumePal E-commerce. Collect order details, display the total bill for confirmation, and save orders to Google Sheets.
